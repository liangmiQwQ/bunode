...
For `node -p` reading the program from stdin, Bunode stores the stdin source in a temporary file and passes Bun a small print expression that reads, evaluates, and removes that file. This preserves large stdin programs without pushing arbitrary user source through OS argv limits.
...
| stdin        | `bun run --no-install --no-env-file ... - ...arguments`        |
