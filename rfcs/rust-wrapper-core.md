...
The binary keeps its option schema based on clap, but parsing is a custom Node-compatible pass.
Node's `-p`, `--`, `NODE_OPTIONS`, and env-file behavior do not map cleanly to clap's normal parser, so clap is used as the shared option definition and validation layer for version-specific option sets.
...
When real `NODE_OPTIONS` is absent, `NODE_OPTIONS` loaded from `--env-file` is parsed before CLI flags, while the `--env-file` flag itself is still forwarded to Bun for normal environment loading.
...
Node builtin module specifiers passed to `--require` or `--import` are kept in `process.execArgv` but are not translated to Bun `--preload`, because Bun expects preload values to resolve as files.
